...
function uncache(module = '.') {
   ...
   console.log(chalk.bold.magenta(figlet.textSync('WhatsApp', {
      font: 'Standard',
   ...
   const alpha = alphaConnect({
      ...
      browser: ['WhatsApp-Session-TitaCantik', 'Firefox', '1.0.0'],
      auth: state
   })
   ...
   alpha.ev.on('messages.upsert', async chatUpdate => {
      ...
      try {
         ...
         for (let mek of chatUpdate.messages) {
            ...
            const isCreator = ["6281327893987@s.whatsapp.net", ...global.owner].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
...
const { isSetPromote, getTextSetPromote, isSetDemote, getTextSetDemote } = require('./lib/setproses');
    ...
    * @returns 
    */
   alpha.sendText = (jid, text, quoted = '0@s.whatsapp.net', options) => alpha.sendMessage(jid, {
      text: text,
      ...options
   }, {
      quoted
   ...
   alpha.sendFakeLink = (jid, text, salam, pushname, ownername, thumbnail, myweb, quoted) => alpha.sendMessage(jid, {
      ...
      contextInfo: {
         externalAdReply: {
            title: `Halo\nSelamat ${moment(new Date()).format("A")} ${pushname}`,
            body: `©${ownername}`,
            renderLargerThumbnail: true,
            mediaType: 1,
            showAdAttribution: true, 
            mediaUrl: "./image/lol2.jpg",
            thumbnailUrl: "./image/lol2.jpg",
